...
"""
Tests for scripts/wake_effects.py — the offshore wind wake model.
"""
...
class TestGetWakeCoefficients:
    ...
    def test_config_overrides(self):
        ...
        assert get_wake_coefficients(config, "tiered_density")["alpha"] == 9.0
...
class TestApplyWakeModel:
    ...
    def test_leaves_onshore_alone(self, tmp_path):
        path = tmp_path / "regions_offshore.geojson"
        _make_regions(["region0"]).to_file(path)
        ...
        assert "onwind gen" in n.generators.index
    ...
    def test_guards_against_double_counting(self):
        n = _make_mock_network()
        config = {
            "electricity": {"wake_model": {"method": "tiered_density"}},
...
class TestResourceClassNaming:
    def test_handles_resource_class_suffix(self):
        n = _make_mock_network(
...
class TestIndexNamesPreserved:
    ...
    def test_preserves_index_names(self):
        n = _make_mock_network(n_generators=1, p_nom_max=[10000.0])
        add_wake_generators(n, {}, _make_regions(["region0"]))
