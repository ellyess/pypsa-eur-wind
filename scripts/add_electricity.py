...
# Variable spatial resolution: prefer split offshore regions when cached
from wake_helpers import get_spatial_mods, get_threshold, get_wake_dir, load_regions
...
def attach_wind_and_solar(
    ...
    for car in carriers:
        ...
        with xr.open_dataset(profile_filenames["profile_" + car]) as ds:
            ...
            # Variable spatial resolution: map sub-region buses to parent buses
            # e.g. "GB0_1" -> "GB0" so generators connect to the electrical bus
            if supcar in ("offwind", "onwind"):
                buses = buses.str.split("_").str[0]
...
if __name__ == "__main__":
    ...
    if wake_method in ("none", "base", None, ""):
        ...
        offwind_mask = n.generators.carrier.str.startswith("offwind")
        ...
        # Density-dependent wake losses need the offshore region areas at the
        # resolution the profiles were built at, i.e. the split regions when
        # variable spatial resolution is active.
        mods = get_spatial_mods(snakemake.config)
        regions_gdf = load_regions(
            "offwind",
            get_threshold(mods, "offwind"),
            get_wake_dir(mods),
