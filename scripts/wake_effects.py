...
This module adds three models, selected via ``electricity: wake_model:
method``. All of them are off by default, so the flat proxy remains in
effect unless a model is explicitly requested.

``uniform``
    A single derate factor on ``p_max_pu`` (default 0.8855). Reproduces the
    magnitude of the existing proxy while moving it out of the profile
    calculation, and serves as the reference case for the other two.

``capacity_tiered``
    A global derate followed by additional marginal losses once a region's
    installed capacity crosses fixed MW thresholds, after Glaum et al.
    Because the thresholds are absolute capacities, the resulting losses
    depend on how finely the offshore regions are resolved.
...
The tiered models are implemented by splitting each offshore generator into
...
References
----------
Glaum, S., Neumann, F., Brown, T. (2024). Offshore power and hydrogen
networks for Europe's North Sea. arXiv:2404.09721 — for the
capacity-tiered approach.

Benmoufok, E. et al. (in preparation) — for the tiered-density model.
...
METHODS = ("uniform", "capacity_tiered", "tiered_density")

DEFAULT_UNIFORM_COEFFICIENTS: dict = {
...
_DEFAULTS = {
    "uniform": DEFAULT_UNIFORM_COEFFICIENTS,
...
def capacity_tiered_wake_spec(coeffs: dict | None = None) -> WakeSplitSpec:
    """
    Build a capacity-tiered wake specification.

    After Glaum et al.: a global derate on all offshore generators, then
    additional marginal losses for capacity beyond fixed MW thresholds.
    """
    if coeffs is None:
    ...
    return WakeSplitSpec(factors=[0.0, f2, f3], max_caps=max_caps_cfg)
...
def apply_uniform_wake(n, config: dict) -> None:
    """Scale offshore ``p_max_pu`` by a single derate factor."""
    derate = get_wake_coefficients(config, "uniform")["derate_factor"]
    offwind = n.generators.index[n.generators.carrier.str.startswith("offwind")]
    if offwind.empty:
        return
    n.generators_t.p_max_pu[offwind] *= derate
    logger.info("Applied uniform wake derate factor %s.", derate)


def add_wake_generators(n, config: dict, method: str, regions_gdf=None) -> None:
    """
    Apply a tiered wake model by splitting offshore wind generators.
    ...
    method : str
        ``"tiered_density"`` or ``"capacity_tiered"``.
    regions_gdf : GeoDataFrame, optional
        Offshore regions with a ``name`` column. Required for
        ``tiered_density``. An ``area`` column in km² is used if present,
        otherwise derived from the geometries.
    """
    if method not in {"tiered_density", "capacity_tiered"}:
    ...
    if method == "tiered_density":
        ...
        global_derate = coeffs["global_derate"]
        n.generators_t.p_max_pu.loc[:, mapping.index] *= global_derate

        min_cap = coeffs["max_caps"][0]
...
def apply_wake_model(n, config: dict, regions_offshore=None) -> None:
    ...
    regions_offshore : path-like, optional
        Offshore regions file. Only read for the ``tiered_density`` method,
        which needs the region areas.
    """
    ...
    if method == "tiered_density":
        if regions_offshore is None:
            raise ValueError(
                "The tiered_density wake model needs the offshore regions."
            )
        regions_gdf = gpd.read_file(regions_offshore)

    add_wake_generators(n, config, method=method, regions_gdf=regions_gdf)
