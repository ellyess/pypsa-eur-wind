...
if __name__ == "__main__":
    ...
    for which in ["regions_onshore", "regions_offshore"]:
        ...
        if clustered.crs is None:
            clustered = clustered.set_crs(epsg=4326)

        # Optional sea-shape clipping for offshore
        sea_shape_path = mods.get("sea_shape")
        if which == "regions_offshore" and sea_shape_path:
            sea = gpd.read_file(sea_shape_path)
            clustered["geometry"] = clustered.geometry.intersection(
                sea.union_all()
            )
        ...
        # Determine technology tag for cache
        tech = "offwind" if "offshore" in which else "onwind"
        try:
            threshold = get_threshold(mods, tech)
        except ValueError:
            threshold = None

        cache_path = regions_file(wdir, tech, threshold)
